...
// Mirrors olevel-quiz.ts's getSubjectPerformanceForStudent/getRecentAttemptsForStudent
// — added once a parent-report module became needed for punjab-9th too. Subject
// here is already a plain human-readable string (no slug-to-label lookup needed,
// unlike O-Level's academy.ts subject registry).
export interface Punjab9thSubjectPerformance {
  subject: string;
  attempts: number;
...
export async function getPunjab9thSubjectPerformanceForStudent(studentEmail: string): Promise<Punjab9thSubjectPerformance[]> {
  await ensureTables();
  ...
  const rows = await sql`
    ...
    FROM punjab9th_quiz_attempts a
    JOIN punjab9th_quizzes q ON q.id = a.quiz_id
...
export async function getPunjab9thRecentAttemptsForStudent(studentEmail: string, limit = 8) {
  await ensureTables();
  const rows = await sql`
