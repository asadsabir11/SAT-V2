...
/** Generates a one-time reset token (valid 1 hour) and stores only its hash. */
export async function createResetToken(email: string): Promise<string> {
  await ensureTable();
  const raw = crypto.randomBytes(32).toString("hex");
  await sql`
    INSERT INTO password_reset_tokens (id, email, token_hash, expires_at)
    VALUES (${crypto.randomUUID()}, ${email.toLowerCase().trim()}, ${hashToken(raw)}, NOW() + INTERVAL '1 hour')
  `;
  return raw;
}

/** Marks a token used and returns the email it was issued for, or null if invalid/expired/already used. */
