...
// Same shape and fallback logic as generateOLevelReportNarrative (both
// programs assess via per-subject quizzes) — a separate function only
// because the AI prompt names the specific curriculum/board, which
// shouldn't say "Cambridge O Level" for a Punjab Board student.
export async function generatePunjab9thReportNarrative(metrics: {
  studentName: string;
  weekNo: number;
  attendanceStatus: string;
  subjects: { subjectLabel: string; attempts: number; avgPercent: number | null }[];
  ...
  const prompt = `You write concise, warm parent progress reports for a Punjab Board 9th Class tuition program.
