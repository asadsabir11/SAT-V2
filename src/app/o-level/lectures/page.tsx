...
function LectureCard({ lec, index }: { lec: Lecture; index: number }) {
  return (
    <Link href={`/lectures/${lec.id}`} style={{ textDecoration: "none" }}>
      <div
        className="card"
...
function OLevelLecturesInner() {
  ...
  const load = useCallback(() => {
    fetch("/api/lectures")
      .then((r) => r.json())
      .then((d) => setLectures((d.lectures ?? []).filter((l: Lecture) => l.program === "o-level")))
  ...
  return (
    <>
      <PageHero eyebrow="O Level materials" title="Lessons">
        Recorded lessons for each O Level subject — watch on demand, rewatch, and take the quiz after each one. Open to any enrolled student, no separate unlock needed.
      </PageHero>
        ...
        <div className="container">
          ...
          {loading ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 20 }}>
              {visible.map((lec, i) => (
                <LectureCard key={lec.id} lec={lec} index={i} />
              ))}
            </div>
          )}
        </div>
