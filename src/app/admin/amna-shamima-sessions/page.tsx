...
export default function AdminAmnaShamimaSessions() {
  const [sessions, setSessions] = useState<AmnaShamimaSession[]>([]);
  ...
  useEffect(() => {
    fetch("/api/admin/amna-shamima-sessions").then((r) => r.json()).then((d) => setSessions(d.sessions ?? [])).finally(() => setLoading(false));
  }, []);

  async function createSession() {
    if (!form.title.trim() || !form.meeting_link.trim() || !form.scheduled_at) return;
    setSaving(true);
    const res = await fetch("/api/admin/amna-shamima-sessions", {
      method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form),
    });
    const d = await res.json();
    setSessions((s) => [{ ...form, id: d.id, created_at: new Date().toISOString() }, ...s]);
    setForm({ ...BLANK });
    setShowForm(false);
    setSaving(false);
  }
  ...
  async function saveEdit(id: string) {
    await fetch(`/api/admin/amna-shamima-sessions/${id}`, {
  ...
  async function toggleActive(s: AmnaShamimaSession) {
    await fetch(`/api/admin/amna-shamima-sessions/${s.id}`, {
  ...
  async function deleteSession(id: string) {
    ...
    await fetch(`/api/admin/amna-shamima-sessions/${id}`, { method: "DELETE" });
      ...
      <div className="container">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12, marginBottom: 28 }}>
          <div>
            ...
            <h1 style={{ fontSize: "1.6rem", fontWeight: 900, color: "#071b33", margin: "6px 0 4px", letterSpacing: "-.03em" }}>Amna Shamima — Online Class</h1>
        ...
        {showForm && (
          <div className="card" style={{ border: "2px solid #059669", marginBottom: 24 }}>
            <h3 style={{ margin: "0 0 18px", color: "#071b33" }}>New online class</h3>
            <div className="form-grid" style={{ marginBottom: 14 }}>
              <div className="field">
                <label>Class title *</label>
                <input value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} placeholder="e.g. Week 3 — Intro to Machine Learning" />
              </div>
              <div className="field">
            ...
            <div className="field" style={{ marginBottom: 14 }}>
              ...
              <input value={form.meeting_link} onChange={(e) => setForm((f) => ({ ...f, meeting_link: e.target.value }))} placeholder="https://zoom.us/j/..." />
            </div>
            <div className="form-grid" style={{ marginBottom: 14 }}>
              <div className="field">
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 16 }}>
              <input type="checkbox" id="active-check" checked={form.is_active} onChange={(e) => setForm((f) => ({ ...f, is_active: e.target.checked }))} style={{ accentColor: "#059669" }} />
              <label htmlFor="active-check" style={{ fontSize: ".88rem", fontWeight: 600, color: "#344054", cursor: "pointer" }}>Visible to students immediately</label>
            </div>
        ...
        {loading ? (
          ...
          <div className="card" style={{ textAlign: "center", padding: 48, color: "#6b7c93" }}>
            ...
            <p style={{ fontWeight: 700, marginBottom: 6 }}>No classes yet</p>
          </div>
        ) : (
          <div style={{ display: "grid", gap: 12 }}>
            {sessions.map((s) => {
              ...
              if (editingId === s.id) {
                return (
                  <div key={s.id} className="card" style={{ border: "2px solid #059669" }}>
              ...
              return (
                <div key={s.id} className="card" style={{ padding: "16px 20px", opacity: isPast && !s.is_active ? 0.6 : 1 }}>
                  <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
                    <div style={{ width: 44, height: 44, borderRadius: 10, background: p.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.3rem", flexShrink: 0 }}>{p.icon}</div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <p style={{ color: "#a0aec0", fontSize: ".78rem", margin: 0 }}>📅 {new Date(s.scheduled_at).toLocaleString(undefined, { dateStyle: "medium", timeStyle: "short" })}</p>
                    ...
                    <div style={{ display: "flex", gap: 6, flexShrink: 0, flexWrap: "wrap", justifyContent: "flex-end" }}>
                      ...
                      <button onClick={() => toggleActive(s)} style={{ padding: "6px 12px", borderRadius: 8, border: "none", fontWeight: 700, fontSize: ".78rem", cursor: "pointer", background: s.is_active ? "#fef3c7" : "#d1fae5", color: s.is_active ? "#92400e" : "#065f46" }}>
                        {s.is_active ? "Hide" : "Show"}
                      </button>
                      <button onClick={() => deleteSession(s.id)} style={{ padding: "6px 12px", borderRadius: 8, background: "#fee2e2", border: "none", color: "#991b1b", fontWeight: 700, fontSize: ".78rem", cursor: "pointer" }}>Delete</button>
                    </div>
                  </div>
                </div>
              );
