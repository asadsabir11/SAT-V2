...
export default function AdminPunjab9thAccess() {
  ...
  useEffect(() => {
    fetch("/api/admin/punjab-9th-access")
      .then(r => r.json())
  ...
  async function grant(email: string) {
    setBusy(email);
    await fetch("/api/admin/punjab-9th-access", {
      method: "POST",
  ...
  async function revoke(email: string) {
    if (!confirm(`Revoke access for ${email}? They will lose full access to their subjects.`)) return;
    setBusy(email);
    await fetch("/api/admin/punjab-9th-access", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    <section className="section">
      <div className="container">
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <h1 style={{ fontSize: "1.6rem", fontWeight: 900, color: "#071b33", margin: "6px 0 4px", letterSpacing: "-.03em" }}>9th Class Access Requests</h1>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 24, flexWrap: "wrap" }}>
          {(["all", "pending", "free", "unlocked"] as Filter[]).map(f => {
            ...
            return (
              <button key={f} onClick={() => setFilter(f)} style={{ padding: "8px 18px", borderRadius: 10, fontWeight: 700, fontSize: ".85rem", cursor: "pointer", border: active ? "2px solid #ea580c" : "2px solid #e8eef6", background: active ? "#fff7ed" : "#f8fafc", color: active ? "#c2410c" : "#6b7c93" }}>
                {label}
              </button>
            );
          })}
                ...
                <tbody>
                  {filtered.map(s => {
                    ...
                    return (
                      ...
                      {isExpanded && (
                        <tr>
                          <td colSpan={8} style={{ background: "#f8fafc", padding: "18px 20px", whiteSpace: "normal" }}>
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "14px 32px" }}>
                              ...
                              <div>
                                ...
                                <div style={{ fontWeight: 700, color: "#071b33" }}>{s.amount_paid ? `PKR ${Number(s.amount_paid).toLocaleString()}` : "—"}</div>
                              </div>
                              <div>
