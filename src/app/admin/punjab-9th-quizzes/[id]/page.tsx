...
export default function Punjab9thQuizEditor({ params }: { params: Promise<{ id: string }> }) {
  return (
    <Suspense fallback={<section className="section"><div className="container"><div className="card"><p>Loading…</p></div></div></section>}>
      <Punjab9thQuizEditorInner params={params} />
    </Suspense>
  );
}

function Punjab9thQuizEditorInner({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    fetch(`/api/punjab-9th/quizzes/${id}`)
      .then((r) => r.json())
      .then((d) => {
        const q = d.quiz;
        if (!q) return;
  ...
  useEffect(() => {
    if (tab === "results") {
      fetch(`/api/punjab-9th/quizzes/${id}/results`)
        .then((r) => r.json())
  ...
  return (
    <section className="section">
      <div className="container">
        <Link href="/admin/punjab-9th-quizzes" style={{ color: "#6b7c93", fontSize: ".82rem", textDecoration: "none" }}>← All 9th Class quizzes</Link>

        <div style={{ display: "flex", alignItems: "center", gap: 10, margin: "12px 0 20px", flexWrap: "wrap" }}>
          <span style={{ padding: "3px 12px", borderRadius: 999, fontSize: ".78rem", fontWeight: 700, background: "#f1f5f9", color: "#475569" }}>{subject}</span>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
          <button onClick={() => setTab("questions")} style={{ padding: "8px 18px", borderRadius: 8, fontWeight: 700, fontSize: ".85rem", cursor: "pointer", border: "none", background: tab === "questions" ? "#ea580c" : "#f1f5f9", color: tab === "questions" ? "#fff" : "#6b7c93" }}>Questions</button>
          <button onClick={() => setTab("results")} style={{ padding: "8px 18px", borderRadius: 8, fontWeight: 700, fontSize: ".85rem", cursor: "pointer", border: "none", background: tab === "results" ? "#ea580c" : "#f1f5f9", color: tab === "results" ? "#fff" : "#6b7c93" }}>Results ({results.length || ""})</button>
        </div>
        ...
        {tab === "questions" ? (
          <>
                  ...
                  {editingId === q.id && editDraft ? (
                    ...
                    <div>
                      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 6, flexWrap: "wrap" }}>
                        ...
                        {q.topic && <span style={{ padding: "2px 10px", borderRadius: 999, fontSize: ".72rem", fontWeight: 700, background: "#fff7ed", color: "#c2410c" }}>{q.topic}</span>}
                      </div>
                      {q.passage && (
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => startEdit(q)} style={{ padding: "6px 12px", borderRadius: 8, background: "#fff7ed", border: "none", color: "#c2410c", fontWeight: 700, fontSize: ".78rem", cursor: "pointer" }}>Edit</button>
            ...
            {!addingQ ? (
              <button className="btn btn-primary" onClick={() => setAddingQ(true)}>
                + Add question
              </button>
            ) : (
              <div className="card" style={{ border: "2px solid #ea580c" }}>
                ...
                <div className="form-grid" style={{ marginBottom: 10 }}>
                  <div className="field"><label>Topic</label><input value={newQ.topic} onChange={(e) => setNewQ({ ...newQ, topic: e.target.value })} placeholder="e.g. Chapter 3" /></div>
