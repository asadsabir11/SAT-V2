...
export default function AdminAmnaShamimaLectures() {
  ...
  function loadLectures() {
    return fetch("/api/admin/amna-shamima-lectures").then((r) => r.json()).then((d) => setLectures(d.lectures ?? []));
  }
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const res = await fetch("/api/admin/amna-shamima-lectures", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title, description, video_url: blob.url, thumbnail_url: thumbnailUrl }),
      });
      if (!res.ok) {
  ...
  async function togglePublish(lec: Lecture) {
    ...
    await fetch(`/api/admin/amna-shamima-lectures/${lec.id}`, {
  ...
  async function deleteLecture(id: string) {
    ...
    await fetch(`/api/admin/amna-shamima-lectures/${id}`, { method: "DELETE" });
    setLectures((ls) => ls.filter((l) => l.id !== id));
  }

  async function saveEdit(id: string) {
    await fetch(`/api/admin/amna-shamima-lectures/${id}`, {
      ...
      <div className="container">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12, marginBottom: 28 }}>
          <div>
            ...
            <h1 style={{ fontSize: "1.6rem", fontWeight: 900, color: "#071b33", margin: "6px 0 4px", letterSpacing: "-.03em" }}>Amna Shamima — Lectures</h1>
        ...
        <div className="card" style={{ marginBottom: 28, border: "2px solid #e8eef6" }}>
          ...
          <div className="form-grid" style={{ marginBottom: 14 }}>
            <div className="field">
              ...
              <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Introduction to AI" />
            </div>
            <div className="field">
          ...
          <div className="field" style={{ marginBottom: 16 }}>
            ...
            <div
              ...
              style={{ border: `2px dashed ${thumbnail ? "#10b981" : "#c8d5e3"}`, borderRadius: 10, overflow: "hidden", cursor: "pointer", background: thumbnail ? "#f0fdf4" : "#f8fafc", minHeight: 80 }}
            >
              {thumbnailPreview ? (
                ...
                {editingId === lec.id ? (
                  ...
                  <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
                    <div style={{ width: 100, height: 64, borderRadius: 8, overflow: "hidden", flexShrink: 0, background: "linear-gradient(135deg,#0c1629,#1e3a5f)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      {lec.thumbnail_url ? <img src={lec.thumbnail_url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} /> : <span style={{ fontSize: "1.5rem" }}>🎬</span>}
                    </div>
                    ...
                    <div style={{ display: "flex", gap: 6, flexShrink: 0, flexWrap: "wrap", justifyContent: "flex-end" }}>
                      <a href={lec.video_url} target="_blank" rel="noreferrer" style={{ padding: "6px 12px", borderRadius: 8, background: "#f1f5f9", border: "none", fontWeight: 700, fontSize: ".78rem", color: "#344054", textDecoration: "none" }}>Preview</a>
                      <button onClick={() => { setEditingId(lec.id); setEditTitle(lec.title); setEditDesc(lec.description); }} style={{ padding: "6px 12px", borderRadius: 8, background: "#eff6ff", border: "none", color: "#155eef", fontWeight: 700, fontSize: ".78rem", cursor: "pointer" }}>Edit</button>
                      <button onClick={() => togglePublish(lec)} style={{ padding: "6px 12px", borderRadius: 8, border: "none", fontWeight: 700, fontSize: ".78rem", cursor: "pointer", background: lec.is_published ? "#fef3c7" : "#d1fae5", color: lec.is_published ? "#92400e" : "#065f46" }}>
                        {lec.is_published ? "Unpublish" : "Publish"}
                      </button>
                      <button onClick={() => deleteLecture(lec.id)} style={{ padding: "6px 12px", borderRadius: 8, background: "#fee2e2", border: "none", color: "#991b1b", fontWeight: 700, fontSize: ".78rem", cursor: "pointer" }}>Delete</button>
                    </div>
                  </div>
                )}
              </div>
