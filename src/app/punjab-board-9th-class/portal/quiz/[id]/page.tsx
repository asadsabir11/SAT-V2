...
export default function TakePunjab9thQuiz({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const [quiz, setQuiz] = useState<Quiz | null>(null);
  const [notFound, setNotFound] = useState(false);
  const [unauthorized, setUnauthorized] = useState(false);
  const [locked, setLocked] = useState<"free" | "pending" | null>(null);
  ...
  useEffect(() => {
    fetch(`/api/punjab-9th/quizzes/${id}`)
      .then((r) => Promise.resolve({ status: r.status, data: r.json() }))
      .then(async ({ status, data }) => {
        const d = await data;
        if (status === 401) { setUnauthorized(true); return; }
        if (status === 403 && d.access) { setLocked(d.access); return; }
  ...
  async function submit() {
    ...
    setSubmitting(true);
    try {
      const r = await fetch(`/api/punjab-9th/quizzes/${id}/submit`, {
        method: "POST",
  ...
  if (unauthorized) return (
    <section className="section"><div className="container">
      <div className="card" style={{ maxWidth: 400, textAlign: "center", padding: 40 }}>
        ...
        <Link href={`/login?role=student&program=punjab-9th&next=/punjab-board-9th-class/portal/quiz/${id}`} style={{ color: "#155eef", fontWeight: 700 }}>Sign in →</Link>
      </div>
    </div></section>
  );
  ...
  if (locked) return (
    <section className="section"><div className="container">
      <div className="card" style={{ maxWidth: 440, textAlign: "center", padding: 40 }}>
        ...
        <p style={{ fontWeight: 700, marginBottom: 8 }}>Full access required.</p>
        <p style={{ color: "#6b7c93", fontSize: ".88rem", marginBottom: 20 }}>
          {locked === "pending" ? "We received your payment — it's being verified." : "Unlock full access to take this quiz."}
        </p>
        {locked !== "pending" && (
          <Link href="/punjab-board-9th-class/portal/unlock" className="btn btn-primary" style={{ marginBottom: 12 }}>Unlock Full Access</Link>
        )}
        <br />
        <Link href="/punjab-board-9th-class/portal" style={{ color: "#155eef", fontWeight: 700, fontSize: ".88rem" }}>← Your subjects</Link>
      </div>
    </div></section>
  );
  ...
  if (notFound || !quiz) return (
    <section className="section"><div className="container">
      <div className="card" style={{ maxWidth: 400, textAlign: "center", padding: 40 }}>
        ...
        <Link href="/punjab-board-9th-class/portal" style={{ color: "#155eef", fontWeight: 700 }}>← Your subjects</Link>
      </div>
    </div></section>
  );
  ...
  return (
    <section className="section">
      <div className="container" style={{ maxWidth: 760 }}>
        <Link href="/punjab-board-9th-class/portal" style={{ color: "#6b7c93", fontSize: ".82rem", textDecoration: "none", display: "inline-block", marginBottom: 16 }}>← Your subjects</Link>
